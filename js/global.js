...
$(function () {
	...
	/*================*/
	var swipers = [],
		winW, winH, winScr, footerTop, _isresponsive, _ismobile = navigator.userAgent.match(/Android/i) || navigator.userAgent.match(/webOS/i) || navigator.userAgent.match(/iPhone/i) || navigator.userAgent.match(/iPad/i) || navigator.userAgent.match(/iPod/i);
	...
	function scrollCall() {
		...
		if ($('.homepage-4-slider-navigation').length) {
			if (winScr + winH - 115 >= footerTop) $('.homepage-4-slider-navigation .rotate').css({
				'margin-top': (-1) * (winScr + winH - footerTop - 115)
			});
			else $('.homepage-4-slider-navigation .rotate').css({
				'margin-top': '0px'
			});
		}
	}
	...
	$('.open-overlay').on('click', function () {
		...
		if ($(this).hasClass('open-video')) setTimeout(function () {
			$('.overlay[data-rel="' + $this.data('rel') + '"] iframe').attr('src', $this.data('src'));
		}, 500);
	...
	$('.tab-menu').on('click', function () {
		...
		$('.tab-entry[data-tab-menu="' + $this.data('tab-menu') + '"]:visible').animate({
			'opacity': '0'
		}, function () {
			$(this).hide();
